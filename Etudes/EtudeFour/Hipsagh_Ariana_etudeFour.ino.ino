...
void live()
{
  ...
  int testNote = resolveToPitch(analogRead(NOTE_IN_PIN));
  if (testNote > 15) {
...
int resolveToPitch(int a) {
  int _t = 0;

  if (a >= 1023 - THRESHOLD) return NOTE_C6;
  else if ((a < 512 + THRESHOLD) && (a >= 512 - THRESHOLD)) return NOTE_D6;
  else if ((a < 341 + THRESHOLD) && (a >= 341 - THRESHOLD)) return NOTE_E6;
  else if ((a < 255 + THRESHOLD) && (a >= 255 - THRESHOLD)) return NOTE_F6;
  else if ((a < 204 + THRESHOLD) && (a >= 204 - THRESHOLD)) return NOTE_G6;
...
void startUpTimer()
{
  ...
  // Serial.println(startTime);    //prints time since program started
...
void updateTimer()
{
  ...
  //  Serial.println(timePassed);    //prints time since program started
  //  delay(1000);             // wait a second so as not to send massive amounts of data
...
void playCurrentNote()
{
  ...
  duration == 100;
  int testNote = resolveToPitch(analogRead(NOTE_IN_PIN));
  if (testNote > peakNote) peakNote = testNote; // this means that we store the highest value always.
  if (testNote > 15) {
    tone(BUZZER_PIN, testNote + offsetFrequency, duration);
    Serial.println(testNote);
  } // wait for tone to finish
...
{
  //IMPLEMENT
  int testphotocell = analogRead(PHOTO_PIN);
  //Serial.println(testphotocell);
  testphotocell = map(testphotocell, 400, 900, 0, 500);
  return testphotocell;
